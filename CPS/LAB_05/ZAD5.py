...
def rysuj_charakterystyke(b, a, passband_hz, stopband_hz, tytul):
    ...
    _, h = freqs(b, a, w)
    h_db = 20 * np.log10(np.abs(h))  # Konwersja na dB
    ...
    plt.legend()
    plt.xlim(0.95 * passband_hz[0], 1.05 * passband_hz[1])
    plt.ylim(-60, 5)
    plt.show()
...
stopband_docelowy = [95.8e6, 96.2e6]

b_docelowy, a_docelowy = projektuj_filtr_analogowy(passband_docelowy, stopband_docelowy, Rp, Rs)

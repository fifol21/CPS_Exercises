...
def manual_resample(x, fs_in, fs_out):
    ...
    numtaps = 599
    cutoff = 0.47 / max(up, down)
    fir_filter = firwin(numtaps, cutoff, window='hamming')
    fir_filter /= np.sum(fir_filter)

    # interpolacja + filtracja
    resampled = upfirdn(fir_filter, x, up=up, down=down)
    
    # wyrównanie opóźnienia filtra
    delay = (numtaps - 1) // 2
    resampled = resampled[delay: -delay or None]
...
for s in signals:
    ...
    resampled_poly.append(x_poly)

min_len = min(map(len, resampled_manual + resampled_poly))
resampled_manual = [x[:min_len] for x in resampled_manual]
resampled_poly = [x[:min_len] for x in resampled_poly]
...
def mix_and_resample(file1, file2, target_sr, play=True):
    with warnings.catch_warnings():
        ...
        sr1, wav1 = wavfile.read('x1.wav')
        sr2, wav2 = wavfile.read('x2.wav')

    wav1_resampled = resample(wav1, int(len(wav1) * target_sr / sr1))
    if wav1_resampled.ndim > 1:
        wav1_resampled = wav1_resampled.mean(axis=1)
    ...
    wav1_resampled = wav1_resampled[:min_length]
